...
const mark = () => '<svg viewBox="0 0 32 32" fill="none" stroke="currentColor" stroke-width="2.3" stroke-linecap="round" aria-hidden="true"><path d="M25 10a12 12 0 1 0 1 10M23 13a7.3 7.3 0 1 0-1 9M19.5 14a3.4 3.4 0 1 0 .2 4"/></svg>';
const brand = () => `<div class="brand"><div class="brand-mark">${mark()}</div><div><strong>Beauty Wallet</strong><small>By ConnectCoin</small></div></div>`;
const orb = () => '<svg viewBox="0 0 240 240" fill="none" stroke="currentColor" stroke-width="1" aria-hidden="true"><circle cx="120" cy="120" r="96"/><circle cx="120" cy="120" r="73"/><ellipse cx="120" cy="120" rx="36" ry="96"/><ellipse cx="120" cy="120" rx="73" ry="96"/><ellipse cx="120" cy="120" rx="96" ry="36"/><ellipse cx="120" cy="120" rx="96" ry="73"/><path d="M24 120h192M120 24v192"/><circle cx="193" cy="82" r="5" fill="#d2ef9b" stroke="#d2ef9b"/><circle cx="74" cy="183" r="4" fill="#b9a5e2" stroke="#b9a5e2"/></svg>';
const format = value => {
...
const cc = value => `${format(value)} CC`;
...
let state = { phase: 'welcome', network: { status: 'disconnected', chain: 'testnet4' }, config: { rpc: { host: 'connectcoin4.com', port: 48190 }, claims: { maxConnectionsPerSecond: 5, maxConcurrent: 5, lookbackBlocks: 600 } } };
let view = 'overview';
...
function setBusy(value) {
  busy = value;
  document.querySelectorAll('[data-busy]').forEach(button => { button.disabled = value || button.dataset.unavailable === 'true'; });
  app.setAttribute('aria-busy', String(value));
}
async function invoke(method, payload = {}) {
  if (!bridge?.invoke) throw new Error('Open Beauty Wallet in the desktop app to use your wallet.');
  ...
  void run(async () => {
    switch (action) {
      ...
      case 'toggle-claims': {
        ...
        await invoke('setClaims', { enabled: !state.claims?.enabled, maxConnectionsPerSecond: Number(config.maxConnectionsPerSecond ?? 5), maxConcurrent: Number(config.maxConcurrent ?? 5), lookbackBlocks: Number(config.lookbackBlocks ?? 600) });
  ...
  void run(async () => {
    switch (form.id) {
      ...
      case 'send-form': {
        ...
        const preview = await invoke('previewSend', { ...destination, amount, feeRate: Number(values.feeRate) }); sendReview(preview); break;
      }
      case 'claims-form': {
